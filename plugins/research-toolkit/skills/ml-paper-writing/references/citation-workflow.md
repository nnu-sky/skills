...
> **Authority rule:** Discovery tools, including Exa and Semantic Scholar, are leads rather than final authority. Verify identity and the attributed claim, then retrieve BibTeX or metadata from an authoritative DOI, publisher, arXiv, or proceedings record. If authoritative metadata cannot be retrieved, leave an explicit placeholder instead of generating a plausible entry.
...
class CitationManager:
    ...
    def get_bibtex(self, paper: Paper) -> Optional[str]:
        ...
        # No authoritative BibTeX route succeeded. Keep this unresolved rather
        # than synthesizing a plausible entry from partial metadata.
        return None
