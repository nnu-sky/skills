...
description: >-
  Create a source-grounded Paper Card when the user requests a literature card
  or comprehensive deep reading of one scientific paper. Do not use for a brief
  summary, a question about one formula, figure or claim, general research ideas,
  translation, or formal peer review.
---
...
Check the requested scope before loading the core. A full Paper Card uses the
Sections 01-16 contract below. A question about one part of a paper, including a
follow-up on an existing card, should be answered directly from the relevant
evidence without generating or re-auditing a full card. Load only the supporting
guidance needed for that question; do not create an artifact unless requested.
...
Load the relevant core on first use; reuse it within the task unless changed or missing. Preserve the evidence-processing steps for each new source.
