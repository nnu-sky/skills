...
Use this before final delivery or a revision package. During iteration, read only
the checks relevant to the change, unresolved defect, statistical claim, or
image-integrity question.

## Validation scope

- Batch related edits. Inspect affected panels or regions, and verify changed data,
  labels and uncertainty; do not rerun the complete suite after every edit.
- Expand checks when shared styles, layout engines or figure dimensions change,
  or when a failure suggests broader effects.
- Before final delivery, run the applicable source, alignment, PDF text and
  collision checks, then inspect every panel and the assembled figure once.
  Correct known defects affecting accuracy, readability or the requested output.
- After a correction, rerun checks invalidated by that correction. A changed final
  PDF needs current PDF checks; unaffected source or layout checks need not be
  repeated. Do not label a stale report as validation of changed output.
- Stop when applicable checks pass and no material defect remains. Automated
  audits supplement scientific and visual judgment; do not pursue subjective
  perfection through repeated unchanged checks.
...
layout, and before exporting submission files. During iteration, rerun when a
change to panel geometry or an observed defect warrants it; final geometry must
be validated before delivery. A single-panel figure is `not applicable`; a
...
Run `audit_figure_collisions.py` on the final PDF before delivery, and during
iteration when a suspected collision or substantial layout change warrants it.
Inspect affected regions for local edits without forcing a full audit cycle.
After fixing a collision, re-export and rerun this check; rerun other checks only
if the correction invalidates them. Do not present a report from an earlier
render as a check of changed final output. Preserve the final JSON report;
