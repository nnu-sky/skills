...
def save_pub_py(fig, filename, dpi=600):
    # Validate final delivery geometry before export. Single-panel
...
- `scripts/audit_figure_collisions.py` — PyMuPDF geometry audit on the final PDF, and for suspected collisions during iteration; fix text-text, text-stroke and clipping FAIL findings before delivery.
- `scripts/figure_safety.py` — strict monotone interpolation and annotation placement above uncertainty extents.

External PDF/image viewers may inspect existing exports without redrawing the scientific content.
