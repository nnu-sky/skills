...
save_pub_r <- function(plot, filename, width_mm = 183, height_mm = 120, dpi = 600) {
  ...
  if (inherits(plot, "patchwork")) {
    # Validate final delivery geometry before export.
...
- `scripts/audit_figure_collisions.py` — backend-neutral geometry audit on the final PDF, and for suspected collisions during iteration; its marked PDF is QA-only and does not replace the R export.
